...
lang_init() {
  ...
  _language.Set("dir", A_ScriptDir "\language")
  _language.Set("fallbacklang", "en-US")
  _language.Set("allLangCodes", Map())
  _language.Set("allLangNames", Map())
  _language.Set("allLangEnNames", Map())
  _language.Set("allLangsFilepath", Map())

  ;Search for languages
  Loop Files, _language.Get("dir") "\*.ini" {
    ...
    if (enlangname != "Error" && langname != "Error" && code != "Error") {
      _language.Get("allLangCodes").Set(code, code)
      _language.Get("allLangNames").Set(code, langname)
      _language.Get("allLangEnNames").Set(code, enlangname)
      _language.Get("allLangsFilepath").Set(code, A_LoopFilePath)
    }
  }
    
  if (!(_language.Get("allLangCodes").Has(_language.Get("fallbacklang")))){
    MsgBox("Fallback language not found in directory: " _language.Get("dir"), "Severe error occured, execution canceled!", "Icon!")
    exitApp
  }
  
  codeSelected := _languageHexToCode.Get(A_Language)  ; Get the name of the system's default language.
  _language.Set("codeSelected", codeSelected)
  
  generateTranslations(IniRead(_language.Get("allLangsFilepath").Get(codeSelected), "translations"))
}
...
langSetLanguage(paramCode := "") {
  ...
  if (paramCode != "") {
    if (!(_language.Get("allLangCodes").Has(paramCode))){
